...
async def test_config_pin_thread(configured_appdaemon: ConfiguredAppDaemonFunc) -> None:
    ...
    app_cfgs = {
        f'test-app-{n}': {
            ...
            'pin_thread': randint(0, additional_ad_cfg['pin_threads'])
        }
        for n in range(1, 10)
    }
    ...
    async with configured_appdaemon(app_cfgs=app_cfgs, **additional_ad_cfg) as (ad, caplog):
...
async def test_callback_pin_thread(configured_appdaemon: ConfiguredAppDaemonFunc) -> None:
    ...
    async with configured_appdaemon(app_cfgs=app_cfgs, **ad_cfg) as (ad, caplog):
        await asyncio.sleep(run_time)
